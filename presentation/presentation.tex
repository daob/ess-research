...
\begin{frame}
 ...
 \begin{columns}<2->[T]
 	\begin{column}{4.5cm}\includegraphics[width=4.5cm]{i/LRV-8cat.pdf}\end{column}
 	\begin{column}<3->{4.5cm}\includegraphics[width=4.5cm]{i/LRV-skew.pdf}
 	\end{column}
 \end{columns}
\end{frame}
...
\begin{frame} 
	...
	\begin{itemize}[<alert@+>]
		...
		\item As method-induced correlation goes up, the estimates of the quality will go down;
		\item If this differs across countries, the process can explain differences in the quality.
	\end{itemize} 
\end{frame}
...
\frametitle{Analysis of the experiments}
	...
	\begin{itemize}[<alert@+>]
		...
		\item The topics: role of women, GP's, political efficacy, job.
			...
			\includegraphics[width=10cm]{i/topics.pdf} 
...
\frametitle{Consequences of correction for categorisation}
	\begin{itemize}[<alert@+>]
		\item In Denmark, the monomethod correlations for the second method increase more than those of the first method and more than the other correlations;
		\item The method effects for the second method are therefore higher, while...
		\item The quality coefficient estimates for the first method are lowered.
		\item In Switzerland, the opposite: the first methods' monomethod correlations increase more on average than the second's, leading to higher quality coefficients after correction.
	\end{itemize}
\end{frame}
...
\frametitle{Correction for categorisation: conclusions}
	...
	\begin{itemize}[<alert@+>]
		\item The general `push' of correcting for categorisation is that all coefficients go up, because the polychoric correlations are always higher than the Pearson correlations;
		\item But when method factors are taken into account, the coefficients can also go down;
		\item Particularly, if one method produces more categorisation errors than another, the quality coefficients can go down;
		\item If this happens more in some countries than others, we can find differences;
...
\begin{frame}
That's it for now. Moltes gràcies per a la seva atenció!

	\begin{columns}[T]
		\begin{column}{3cm}\includegraphics[width=2.55cm]{i/uvt_fac.jpg}\end{column}
		\begin{column}{3cm}\includegraphics[width=2.5cm]{i/esade_fac.jpg}\end{column}
	\end{columns}		
