...
\begin{table}[thb]\centering\caption{Quality according to the continuous and categorical models, with categorisation factors for the experiment on opinions about the role of men and women in society.\label{tab:women}}\begin{tabular}{lllrrr}
\hline
...
\begin{table}[htb]\centering\caption{Quality according to the continuous and categorical models, with categorisation factors for the experiment on certain aspects of the job of the respondent.\label{tab:job}}\begin{tabular}{lllrrr}
\hline
  &    & & \multicolumn{3}{c}{`Job'} \\
 &      &   & Varied & Secure & Risky\\
\hline
...
\begin{table}[hbt]\centering\caption{Quality according to the continuous and categorical models, with categorisation factors for the experiment on perception of the behaviour of doctors.\label{tab:doctors}}\begin{tabular}{lllrrr}
\hline
...
The same does not happen in Denmark though; here the fourth question more or less symmetrical thresholds. This is somewhat surprising given the thresholds of the same question using a direct question rather than an agree-disagree scale; if the scales were equally good but simply a reverse way of phrasing then one would expect also a reversal of the thresholds. The fact that this does not occur suggests that other factors play a part besides the categorisation errors, such as the quality coefficients, random errors and method variance.
...
\begin{table}[htb]\centering\caption{Quality according to the continuous and categorical models, with categorisation factors for the experiment on the respondent's subjective political competence (political efficacy).\label{tab:efficacy}}\begin{tabular}{lllrrr}
\hline
 &  &  &  & `Efficacy' & \\
	   &  &  & Complex & Active & Mind\\
	   \hline
...
Another consequence of this can be seen for the `active' question. Though in Switzerland and Denmark similar overall connections to the corresponding trait according to the continuous analysis were found, this similarily was apparently produced from very dissimilar categorisation errors and quality coefficients.
...
Using the results presented in the previous sections, we constructed a data set consisting of the categorisation factor for all questions--including those from the supplementary questionnaire not shown above--in the four different experiments for which this index was available. This yielded 78 cases in total. We also recorded the country, topic of the experiment, the type of scale used (agree-disagree, true-false or direct question) and whether the question was phrased as a negative statement or not. The categorisation factors analysed are presented as a histogram in figure \ref{fig:hist_factor}.

\begin{figure}[htb]\caption{Histogram of the categorisation factor.\label{fig:hist_factor}}\centering
\includegraphics[width=5.5cm]{hist_factor}
\end{figure}

In order to see what differences could be detected across these variables with respect to the categorisation factor while controlling for the others, we estimated a linear regression of the categorisation factors on country, topic, scale and negatively phrased question or not. An examination of the residuals suggests that there are no large deviations from the assumptions of linearity of the relationships and normality of the residuals\footnote{The largest standardised residual equals .34.}. The overall amount of variance in explained is 55\%. The estimates can be found in table \ref{tab:meta}.
