...
This split-ballot MTMM approach lightens the response burden by presenting fewer questions and fewer repetitions. \citet{saris_new_2004} showed that the different parameters of the MTMM model can still be estimated using this planned missing data design. If the covariances between the questions belonging to the first and second, between those from the first and third methods and the covariances between questions using the same method are identified, then so are the different parameters of the MTMM model. In the categorical MTMM model, these covariances are indeed also identified, and therefore \citep{millsap_assessing_2004} the entire model is as well.
...
\multicolumn{2}{l}{Categorical analysis  } &  &  &  & \\
 ...
 & $m$ & Denmark & 0.49 & 0.17 & 0.17\\
 &  & Estonia & 0.48 & 0.37 & 0.42\\
\multicolumn{2}{l}{Categorisation factor}   &  &  &  & \\
...
Not everything about categorisation error can directly deduced by looking only at the thresholds. The categorical model produced results which might seem surprising if this is not taken into account. The likelihood ratio in this case was 26 with 21 degrees of freedom ($p=.19$). Table \ref{tab:doctors} shows the results we obtained for this experiment.
...
In Estonia the quality using the continuous model is also estimated to be rather low. However, the categorical model estimates the quality of the underlying scale at a level similar to that of the other two questions.
...
The comparison between the categorical and continuous models for this experiment is shown in table \ref{tab:efficacy}.
...
Using the results presented in the previous sections, we constructed a data set consisting of the categorisation factor for all questions--including those from the supplementary questionnaire not shown above--in the four different experiments for which this index was available. We also added data on questions from Switzerland and the Czech Republic for the `women' experiment. This yielded 78 cases in total. We also recorded the country, topic of the experiment, the type of scale used (agree-disagree, true-false or direct question) and whether the question was phrased as a negative statement or not. The categorisation factors analysed are presented as a histogram in figure \ref{fig:hist_factor}. Figure \ref{fig:box_factor} shows a plot of the quality obtained in the continuous analysis and the categorisation factor.
...
\hspace{0.5cm} 
\begin{minipage}[b]{0.5\linewidth}
\centering\includegraphics[width=6cm]{box_factor.pdf}
\caption{Scatterplot of $q^2_{continuous}$ and the categorisation factor.\label{fig:box_factor}}
\end{minipage}
\end{figure}

The categorisation factor is of course calculated using $q^2_{cont.}$, but a relationship is only to be expected when the qualities under the categorical and continuous models are not constantly proportional to each other. Figure \ref{fig:box_factor} shows that with one exception, categorisation factors above one occur only when the quality is relatively high. It can also be seen that the absolute level of the quality and the discrepancies between the quality under the two models are related. If the sample is split according to whether the quality was `high'  or `low' (above or below the median 0.56), the means of the two groups are 1.11 and 0.73, respectively ($t=5.7$, $df \approx 38$, $p<.00001$).

In order to see what differences in the categorisation factors could be detected, we estimated a linear regression of the categorisation factors on country, topic, scale and whether the question was negatively phrased or not. An examination of the residuals suggests that there are no large deviations from the assumptions of linearity of the relationships and normality of the residuals\footnote{The largest standardised residual equals .34.}. The overall amount of variance in explained is 55\%. The estimates can be found in table \ref{tab:meta}.
...
\begin{table}[htb]\centering\caption{A meta-analysis of the categorisation error studies. The table presents a linear regression of the categorisation factor on topic, country, scale and scale position.\label{tab:meta}}
\begin{minipage}[ht]{.73\textwidth}
\begin{tabular}{llrrrr}
\hline
 &  & Estimate & S.E. & lower & upper\\
 \hline
 & (Intercept) & 0.53 & 0.19 & 0.16 & 0.90 \\
\multicolumn{2}{l}{\textit{Topic}}&  &  &  & \\
 & Doctors & (reference category) &  &  & \\
 & Efficacy & 0.20 & 0.09 & 0.03 & 0.37  \\
 & Job & 0.23 & 0.19 & -0.13 & 0.58  \\
 & Women & 0.51 & 0.13 & 0.25 & 0.78 \\
\multicolumn{2}{l}{\textit{Country  } }&  &  &  & \\
 & Belgium & (reference category) &  &  & \\
 & Slovenia & 0.09 & 0.10 & -0.11 & 0.28 \\
 & Czech Republic & 0.28 & 0.15 & -0.02 & 0.59 \\
 & Switzerland & 0.31 & 0.15 & 0.01 & 0.60 \\
 & Greece & 0.32 & 0.15 & 0.01 & 0.62  \\
 & Denmark & 0.33 & 0.17 & -0.01 & 0.67 \\
 & Estonia & 0.53 & 0.19 & 0.15 & 0.91 \\
\multicolumn{2}{l}{\textit{Scale   }}&  &  &  & \\ 
 & Agree-disagree & -0.16 & 0.07 & -0.29 & -0.03 \\
 & Direct  & (reference category) &  &  & \\
 & True-false & 0.30 & 0.11 & 0.08 & 0.51 \\
\multicolumn{2}{l}{\textit{Scale position}   }&  &  &  & \\
 & Negative & -0.50 & 0.07 & -0.62 & -0.33 \\
\hline
\end{tabular}
Multiple R-Squared: 0.55; Adjusted R-squared: 0.46; F-test for factor sums of squares: 
Topic: $p=.15$; Country: $p=.15$; Scale: $p=.01$; Position: $p<.00001$.
\end{minipage}
\end{table}
...
The country explains little of the variation in the categorisation factor. The exception however, is Estonia where a large coefficient was found which is likely to be different from zero. The average categorisation factor for this country keeping all other variables at zero is above one, indicating that the quality under the categorical model in this country was lower than the same quality estimate under the continuous model, scale, negative phrasing, and topic being equal. We should also note the higher standard errors for these coefficients.
% Given the relationship between the categorisation factor and the quality and the fact that the quality differs across the countries, it is likely that the coefficients are in fact higher than a statistical test would indicate at this point\footnote{A bayesian analysis of this model using uniform priors on the coefficients suggests that the probability that the country effects are larger than 0.1 is around .8.}.
