...
dat <- read.table("C:\\ESS\\berlin\\output\\women-smallmodel-covariances-R", header=F)
dat <- dat[,1:2]
...
for(cntry in 1:length(countries)) {
  ...
  write.table(covars[[cntry]], paste(outpath,'women_',countries[cntry],'.cov',sep=""),row.names=F,col.names=F,na="" )
    write.table(means[[cntry]], paste(outpath,'women_',countries[cntry],'.mean',sep=""),row.names=F,col.names=F,na="" )
