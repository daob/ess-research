...
Using the results presented in the previous sections, we constructed a data set consisting of the categorisation factor for all questions--including those from the supplementary questionnaire not shown above--in the four different experiments for which this index was available. This yielded 72 cases in total. We also recorded whether the question was asked in the country with the highest or lowest quality, was in the main or supplementary questionnaire, the topic of the experiment, the type of scale used (agree-disagree, true-false or direct question) and whether the question was phrased as a negative statement or not. The categorisation factors analysed are presented as a histogram in figure \ref{fig:hist_factor}. Figure \ref{fig:box_factor} shows a boxplot of the categorisation factor for the countries with the highest and lowest quality in each experiment, separated by whether the question was asked in the main or supplementary questionnaire.
...
In order to see whether this explanation of the differences holds when we control for other variables, we estimated a linear regression of the categorisation factors on whether the question was asked in a country with a high or low quality, was in the main or supplementary questionnaire, its topic, its scale and whether the question was negatively phrased or not. The amount of adjusted variance explained is 35\%. The estimates can be found in table \ref{tab:meta}.
...
\begin{table}[htb]\centering\caption{A meta-analysis of the categorisation error studies. \label{tab:meta}}
...
The interaction effect shown in figure \ref{fig:box_factor} has not been suppressed while controlling for other variables. Table \ref{tab:meta} shows that except for negative phrasing and asking the question in the country with the highest quality, the categorisation factor does not differ significantly for any of the other variables than this interaction.

Some questions were negatively phrased, for example as `A woman should \emph{not} have to cut down on her paid work for the sake of her family'. For such questions, the quality estimate in the continuous analysis was consistently lower than the quality obtained in the categorical analysis.
...
A test was done of the hypothesis that questions for which the method effect was constrained to zero in the continuous model have the same categorisation factor as other questions, controlling for country effects. This hypothesis was rejected ($p=0.02$)\footnote{Result of a hierarchical linear model fit using \texttt{R 2.6.1} with fixed effects of country and restricting the method to zero or not (0/1), and a random intercept across topics to account for the dependency among the observations.}. This explanation therefore seems plausible.  

\begin{figure}[bth] \centering \includegraphics[width=.6\textwidth]{i/meta-scatter} \caption{Scatterplot of the categorisation factor ($c$) and the total quality of a measure ($q_{cont.}^2$) across the experiments.\label{fig:meta_scatter}} \end{figure}

Figure \ref{fig:meta_scatter} shows that categorisation factors above unity were mostly found for questions with a high quality. If the quality coefficients from the continuous and categorical models were more or less equal, the categorisation factor would be constant across the continuous models' quality. After all, it simply equals $c := q^2_{cont}/q^2_{cat}$. The relationship that can be seen in the figure indicates that high quality coefficients from the continuous model tend to be lower in the categorical model, and vice versa.
